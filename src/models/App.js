...
const AppStore = types
  ...
  .views(self => ({
    get won() {
      const won =
        self.totalUnveiled ===
        self.rowCount * self.columnCount - self.mineCount;
      if (won) self.stopTimer();
      return won;
    },
    get bestScoreMin() {
      return self.best.length > 0
        ? Math.max(...self.best.map(score => score.value))
        : 99999999999999;
    },
    get bestSorted() {
  ...
  .actions(self => ({
    ...
    generateMines() {
      ...
      while (arr.length < self.mineCount) {
        const randomnumber =
          Math.floor(Math.random() * (self.rowCount * self.columnCount)) + 1;
    ...
    unveilAdjacentBlanks({id, leftEdge, rightEdge}) {
      ...
      adjacent
        .filter(a => a >= 0 && a <= self.cells.length - 1)
        .forEach(cellId => {
